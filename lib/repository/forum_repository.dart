...
class ForumRepository {
  static Future<int> countForumPost({@required String forumId}) async {
    var result =
        await BaseRepository.selectClient.countForumPost(forumId: forumId);
    return result.data['countForumPost'];
  }

  static Future<List<ForumModel>> getForums(
      {int page = 1, int limit = 10}) async {
    var result =
        await BaseRepository.selectClient.getForums(page: page, limit: limit);
    return Forums.fromList(result.data['getForums']).forums;
  }

  static Future<List<HashTagModel>> getHashTagStats() async {
    var result = await BaseRepository.selectClient.getHashTagStats();
    return HashTagModels.fromList(result.data['getHashTagStats']).hashtags;
  }
}
