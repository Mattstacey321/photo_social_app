...
class PostRepository {
  static Future<List<PostModel>> getForumPost({String forumId, int page, int limit}) async {
    var result =
        await BaseRepository.selectClient.getForumPost(forumId: forumId, page: page, limit: limit);
    return PostsModel.fromList((result).data['getForumPost']).posts;
  }

  static Future<bool> likePost({String postId, String forumId}) async {
    var result = await BaseRepository.selectClient.likePost(postId: postId, forumId: forumId);
